...
#define FCALC_LOOPS         500000UL
#define FINITIAL            1.1234556789
#define FMAC                1.127654321
#define FDIV                1.123467789

//comment it if you want to make button 1 as non-vector interrupt
#define SOC_BUTTON_1_HANDLER_IS_VECTOR
//comment it if you want to make button 2 as non-vector interrupt
//#define SOC_BUTTON_2_HANDLER_IS_VECTOR

#ifdef SOC_BUTTON_1_HANDLER_IS_VECTOR
// Vector interrupt
...
#endif
{
    ...
    SAVE_FPU_CONTEXT();
#endif /* __riscv_flen */
#endif /* SOC_BUTTON_1_HANDLER_IS_VECTOR */

#ifdef CFG_SIMULATION
    write(STDOUT_FILENO, "----Begin button1 handler\n",
          strlen("----Begin button1 handler\n"));
#else
    printf("%s", "----Begin button1 handler, high priority----\n");
#endif
...
#ifdef SOC_BUTTON_2_HANDLER_IS_VECTOR
// Vector interrupt
__INTERRUPT void SOC_BUTTON_2_HANDLER(void)
#else
// Non Vector interrupt
...
#endif
{
    ...
    SAVE_FPU_CONTEXT();
#endif /* __riscv_flen */
#endif /* SOC_BUTTON_2_HANDLER_IS_VECTOR */

#ifdef CFG_SIMULATION
    write(STDOUT_FILENO, "----------Begin button2 handler\n",
          strlen("----------Begin button2 handler\n"));
#else
    printf("%s", "--------Begin button2 handler, low priority----\n");
#endif
...
void config_eclic_irqs()
{
    ...
    ECLIC_SetShvIRQ(SOC_BUTTON_1_IRQn, ECLIC_VECTOR_INTERRUPT);
    printf("Configure SOC Button 1 interrupt to be vector interrupt\r\n");
#else
    ECLIC_SetShvIRQ(SOC_BUTTON_1_IRQn, ECLIC_NON_VECTOR_INTERRUPT);
    printf("Configure SOC Button 1 interrupt to be non-vector interrupt\r\n");
#endif

#ifdef SOC_BUTTON_2_HANDLER_IS_VECTOR
    //  The BUTTON2 using Vector-Mode
    ECLIC_SetShvIRQ(SOC_BUTTON_2_IRQn, ECLIC_VECTOR_INTERRUPT);
    printf("Configure SOC Button 2 interrupt to be vector interrupt\r\n");
#else
    ECLIC_SetShvIRQ(SOC_BUTTON_2_IRQn, ECLIC_NON_VECTOR_INTERRUPT);
    printf("Configure SOC Button 2 interrupt to be non-vector interrupt\r\n");
#endif


    // Have to enable the interrupt both at the GPIO level,
    // and at the ECLIC level.
    ECLIC_EnableIRQ(SOC_BUTTON_1_IRQn);
    ECLIC_EnableIRQ(SOC_BUTTON_2_IRQn);
}
