...
#define TEST_BLOCK_MEMORY_PAYLOAD_SIZE          100
#define TEST_BLOCK_MEMORY_BLOCK_COUNT           3
#define TEST_BLOCK_MEMORY_ALIGNED_BLOCK_SIZE    ((((TEST_BLOCK_MEMORY_PAYLOAD_SIZE) + sizeof(ALIGN_TYPE) - 1) / sizeof(ALIGN_TYPE)) * sizeof(ALIGN_TYPE))
#define TEST_BLOCK_MEMORY_POOL_BYTES            ((TEST_BLOCK_MEMORY_ALIGNED_BLOCK_SIZE + sizeof(UCHAR *)) * TEST_BLOCK_MEMORY_BLOCK_COUNT)
...
INT     status;
    ...
    /* Create block pool 0.  */
    status =  tx_block_pool_create(&pool_0, "pool 0", TEST_BLOCK_MEMORY_PAYLOAD_SIZE, pointer, TEST_BLOCK_MEMORY_POOL_BYTES);
    pointer = pointer + TEST_BLOCK_MEMORY_POOL_BYTES;
    ...
    status =  tx_block_pool_create(TX_NULL, "pool 0", TEST_BLOCK_MEMORY_PAYLOAD_SIZE, pointer, TEST_BLOCK_MEMORY_POOL_BYTES);
    if (status != TX_POOL_ERROR)
    {
    ...
    status =  tx_block_pool_create(&pool_1, "pool 0", TEST_BLOCK_MEMORY_PAYLOAD_SIZE, NULL, TEST_BLOCK_MEMORY_POOL_BYTES);
...
INT     i;
    ...
    status =  tx_block_pool_create(&pool_1, "pool 1", TEST_BLOCK_MEMORY_PAYLOAD_SIZE, pointer, TEST_BLOCK_MEMORY_POOL_BYTES);
    pointer = pointer + TEST_BLOCK_MEMORY_POOL_BYTES;

    /* Attempt to create a pool with an invalid size.  */
    status =  _txe_block_pool_create(&pool_2, "pool 2", TEST_BLOCK_MEMORY_PAYLOAD_SIZE, pointer, TEST_BLOCK_MEMORY_POOL_BYTES, 777777);
    if (status != TX_POOL_ERROR)
    {
