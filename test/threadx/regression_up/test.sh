...
# 常见用法：
#   ./test.sh <host:port>
#   WAIT_SECONDS=15 ./test.sh whss6:22000
#   BUILD_ONLY=1 CORE=n900fd ./test.sh
#   CASES="threadx_thread_simple_sleep_test" ./test.sh whss6:22000
...
可用环境变量：
  ...
  CORE           传给 make 的 CORE 值，默认: n900fd
  BUILD_ONLY     设为 1 时只构建，不 upload、不做 GDB 探测，默认: 0
  WAIT_SECONDS   upload 后等待目标运行时间，默认: 15
  GDB_TIMEOUT    每次 GDB 会话超时时间，默认: 15
  TARGET_ELF     GDB 使用的 ELF，默认: threadx_regression_up.elf
  ELF_OUTPUT_DIR 每个 case 的 ELF 归档目录，默认: elfs/\${CORE}
  LOG_DIR        显式指定日志目录
...
TARGET_ELF=${TARGET_ELF:-threadx_regression_up.elf}
ELF_OUTPUT_DIR=${ELF_OUTPUT_DIR:-elfs/${CORE}}
...
{
    local list_file=$1
    local verdict_pattern=

    case "${CASE_SET}" in
        "")
            ;;
        not-finished)
            verdict_pattern='^\[NOT-FINISHED\] '
            ;;
        fail)
            verdict_pattern='^\[FAIL\] '
            ;;
        exception)
            verdict_pattern='^\[EXCEPTION\] '
            ;;
        skip)
            verdict_pattern='^\[SKIP\] '
            ;;
        pass)
            verdict_pattern='^\[PASS\] '
            ;;
        *)
    ...
    if [ -n "${verdict_pattern}" ]; then
        awk -v pattern="${verdict_pattern}" '
            $0 ~ pattern {
                case_name = $2
                if (case_name != "") {
...
while IFS= read -r case_path; do
    ...
    printf 'Test on %s (timeout=%ss)\n' "${case_path}" "${case_wait}"
    {
        echo "=== ${case_name} ==="
        echo "case: ${case_path}"
        echo "elf_output: ${ELF_OUTPUT_DIR}/${case_name}.elf"
        echo "gdbremote: ${GDBREMOTE}"
    ...
    if ! make GDBREMOTE="${GDBREMOTE}" CORE="${CORE}" SILENT=1 clean >>"${upload_log}" 2>&1; then
    ...
    if [ "${BUILD_ONLY}" = "1" ]; then
        if ! make CORE="${CORE}" CTEST="${case_path}" SILENT=1 -j all >>"${upload_log}" 2>&1; then
    ...
    elif ! make GDBREMOTE="${GDBREMOTE}" CORE="${CORE}" CTEST="${case_path}" SILENT=1 -j upload >>"${upload_log}" 2>&1; then
