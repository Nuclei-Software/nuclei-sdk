...
#define TEST_BLOCK_POOL_BYTES 336
#else
#define TEST_BLOCK_POOL_BYTES 320
#endif
...
INT     status;
    ...
    /* Create block pool 0.  */
    status =  tx_block_pool_create(&pool_0, "pool 0", 100, pointer, TEST_BLOCK_POOL_BYTES);
    pointer = pointer + TEST_BLOCK_POOL_BYTES;
    ...
    status =  tx_block_pool_create(TX_NULL, "pool 0", 100, pointer, TEST_BLOCK_POOL_BYTES);
    if (status != TX_POOL_ERROR)
    {
    ...
    status =  tx_block_pool_create(&pool_1, "pool 0", 100, NULL, TEST_BLOCK_POOL_BYTES);
...
INT     i;
    ...
    status =  tx_block_pool_create(&pool_1, "pool 1", 100, pointer, TEST_BLOCK_POOL_BYTES);
    pointer = pointer + TEST_BLOCK_POOL_BYTES;

    /* Attempt to create a pool with an invalid size.  */
    status =  _txe_block_pool_create(&pool_2, "pool 2", 100, pointer, TEST_BLOCK_POOL_BYTES, 777777);
    if (status != TX_POOL_ERROR)
    {
