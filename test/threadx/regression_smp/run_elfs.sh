...
# Run generated ThreadX regression ELF files on QEMU or xlmodel.
#
...
Environment variables:
  ...
  ELF_DIR       Directory containing case ELF files, default: elfs/\${CORE}_smp\${SMP}
  CASES         Space-separated case names or ELF paths
  LOG_DIR       Explicit log directory, default: logs/run_elfs_<runner>_<core>_smp<smp>_<timestamp>
...
Examples:
  ...
  PARALLEL=4 RUNNER=qemu CORE=n900fd SMP=4 ./run_elfs.sh
  RUNNER=xlmodel CORE=nx900fd SMP=4 TIMEOUT=30 ./run_elfs.sh
  RUNNER=board GDBREMOTE=whss6:22000 CORE=n900fd SMP=4 ./run_elfs.sh
  CASES="threadx_smp_time_slice_test threadx_mutex_basic_test" ./run_elfs.sh
EOF
}
...
ELF_DIR=${ELF_DIR:-elfs/${CORE}_smp${SMP}}
CASES=${CASES:-}
RUN_ID=${RUN_ID:-$(date +%Y%m%d_%H%M%S)}
LOG_DIR=${LOG_DIR:-logs/run_elfs_${RUNNER}_${CORE}_smp${SMP}_${RUN_ID}}
...
build_run_cmd()
{
    ...
    case "${RUNNER}" in
        qemu)
            printf '%s -M nuclei_evalsoc,download=%s -cpu nuclei-%s,ext= -smp %s -nodefaults -nographic -serial stdio -kernel %s\n' \
                "${EMULATOR}" "${DOWNLOAD}" "${CORE}" "${SMP}" "${elf_path}"
            ;;
        xlmodel)
            printf '%s -M nuclei_evalsoc --cpu=%s --download=%s --smp=%s %s\n' \
                "${EMULATOR}" "${CORE}" "${DOWNLOAD}" "${SMP}" "${elf_path}"
            ;;
    esac
}
...
stop_run_pid()
{
    ...
    # xlmodel may keep helper processes alive after the test already printed
    # SUCCESS, so terminate the whole process group instead of only the shell.
...
run_one_elf()
{
    ...
    # Run each emulator invocation in its own session so PASS/FAIL detection
    # can stop the entire process group cleanly on qemu and xlmodel.
